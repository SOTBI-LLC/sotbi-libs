...
export class FlowState implements NgxsOnInit {
  ...
  public fetchItems({ getState, setState }: StateContext<SimpleEdit2StateModel>) {
    ...
    if (!state.items.length) {
      return this.itemsService.getAll(SimpleEdit2ServiceNames.FLOW).pipe(
        tap((result) => {
  ...
  @Action(AddItem)
  public addItem({ getState, setState }: StateContext<SimpleEdit2StateModel>, { payload }) {
    const state = getState();
  ...
  @Action(EditItem)
  public editItem({ getState, setState }: StateContext<SimpleEdit2StateModel>, { payload }) {
    const state = getState();
    const { id, ...rest } = payload;
    return this.itemsService.save(id, rest, SimpleEdit2ServiceNames.FLOW).pipe(
      tap((selected) => {
        const mapFItems = { ...state.mapFItems };
        const mapTItems = { ...state.mapTItems };
        if (selected.kind) {

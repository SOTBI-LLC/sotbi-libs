...
import { AddItem, DeleteItem, EditItem, FetchAttachmentTypes } from './attachmenttypes.actions';
...
export class AttachmentTypesState {
  ...
  public fetchItems({ getState, patchState }: StateContext<SimpleEditStateModel>) {
  ...
  public addItem({ getState, patchState }: StateContext<SimpleEditStateModel>, { payload }) {
    const state = getState();
    return this.itemsService.create(payload.name, SimpleEditServiceNames.ATTACHMENT).pipe(
      tap((result) => {
  ...
  public editItem({ getState, patchState }: StateContext<SimpleEditStateModel>, { payload }) {
    const state = getState();
    return this.itemsService.save$(payload, SimpleEditServiceNames.ATTACHMENT).pipe(
      tap(
        (result) => {
          const mapItems = state.mapItems;
          mapItems.set(result.id, result.name);
  ...
  public deleteItem({ getState, patchState }: StateContext<SimpleEditStateModel>, { payload }) {
    const state = getState();
    return this.itemsService.delete(payload, SimpleEditServiceNames.ATTACHMENT).pipe(
      tap(() => {
        const mapItems = state.mapItems;
